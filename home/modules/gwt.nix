...
in {
  options.programs.gwt = {
    enable = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Enable Git Worktree Management System";
    };

    worktreeBase = lib.mkOption {
      type = lib.types.str;
      default = "$HOME/dev/worktrees";
      description = "Base directory for git worktrees";
      example = "$HOME/projects/worktrees";
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    ...
    programs.zsh = {
      initContent = lib.mkOrder 1100 ''
        ...
        __gwt_lock() {
          local lockfile="/tmp/gwt.lock"
          exec 200>"$lockfile"
          flock -n 200 || {__gwt_print_error "Another gwt instance is running. Exiting."; return 1; }
        }
        ...
        __gwt_get_worktree_base() {
          ...
          echo "${cfg.worktreeBase}/$project_name"
        }
        ...
        __gwt_analyze_issues() {
          ...
          for issue in "''${issues[@]}"; do
            ...
            if [ $? -ne 0 ]; then
              ...
              local title
              title=$(printf '%s\n' "$info" | jq -r '.title // "issue-'$issue'"' 2>/dev/null || echo "issue-$issue")
              titles+=("$title")
          ...
          if [ ''${#issues[@]} -eq 1 ]; then
            ...
            if [ ''${#labels_all[@]} -gt 0 ]; then
              ...
              for labels in "''${labels_all[@]}"; do
                component_labels+=($(echo "$labels" | rg -oE '\b(frontend|backend|api|ui|auth|database)\b' || true))
                type_labels+=($(echo "$labels" | rg -oE '\b(bug|feature|enhancement|refactor)\b' || true))
              done
